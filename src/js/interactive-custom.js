...
Physics.behavior('interactive-custom', function(parent) {
    ...
    maxVel : {x : 5, y : 5}
      ...
      getElementOffset = function(el) {
        ...
        let offset = getElementOffset(e.target),
            ...
            x = obj.pageX - offset.left, y = obj.pageY - offset.top;
  ...
  return {
    ...
    init : function(options) {
      let self = this, prevTreatment, time;
      ...
      let grab = function grab(e) {
        let pos = getCoords(e), body;

        time = Physics.util.ticker.now();

        if (self._world) {
          body = self._world.findOne({
            $at : new Physics.vector(pos.x, pos.y),
            $in : self.getTargets()
          });
          ...
          if (body) {
            ...
            body =
                self._world.findOne({$at : new Physics.vector(pos.x, pos.y)});
            if (body) {
              pos.body = body;
      ...
      let release = function release(e) {
        let pos = getCoords(e), body,
            dt = Math.max(Physics.util.ticker.now() - time,
                          self.options.moveThrottle);
        ...
        if (self.body) {
          ...
          self.body.state.vel.clone(self.mousePos)
              .vsub(self.mousePosOld)
              .mult(1 / dt);
    ...
    behave : function(data) {
      let self = this, state, dt = Math.max(data.dt, self.options.moveThrottle);

      if (self.body) {

        // if we have a body, we need to move it the the new mouse position.
        // we'll do this by adjusting the velocity so it gets there at the next
        // step
        state = self.body.state;
        state.vel.clone(self.mousePos)
            .vsub(self.offset)
            .vsub(state.pos)
            .mult(1 / dt);
      }
